...
package org.si.cml {
    /** @private */
    internal class CMLState extends CMLListElem
    {
    // variables
    //------------------------------------------------------------
        ...
        static internal const ST_W4D     :uint =14;   // wait for destruction
        ...
        // invert flag
        static internal var _invert_flag:uint = 0;
        
        // speed ratio
        static internal var _speedRatio:Number = 1;
    ...
    //------------------------------------------------------------
        ...
        internal override function _clear() : void
        {
            _args.length = 0;
            jump = null;
            super._clear();
    ...
    //------------------------------------------------------------
        ...
        // velocity
        private function _v(fbr:CMLFiber)  : Boolean { fbr.object.setVelocity(_invertX(_args[0]*_speedRatio), _invertY(_args[1]*_speedRatio), fbr.chgt); return true; }
        private function _vx(fbr:CMLFiber) : Boolean { fbr.object.setVelocity(_invertX(_args[0]*_speedRatio), fbr.object.vy,                  fbr.chgt); return true; }
        private function _vy(fbr:CMLFiber) : Boolean { fbr.object.setVelocity(fbr.object.vx,                  _invertY(_args[0]*_speedRatio), fbr.chgt); return true; }
        private function _vd(fbr:CMLFiber) : Boolean {
            ...
            var c:Number = sin[iang+sin.cos_shift],
                s:Number = sin[iang],
                h:Number = _args[0] * _speedRatio,
                v:Number = _args[1] * _speedRatio;
            fbr.object.setVelocity(c*h-s*v, s*h+c*v, fbr.chgt);
            return true;
        }

        // acceleration
        private function _a(fbr:CMLFiber)  : Boolean { fbr.object.setAccelaration(_invertX(_args[0]*_speedRatio), _invertY(_args[1]*_speedRatio), 0); return true; }
        private function _ax(fbr:CMLFiber) : Boolean { fbr.object.setAccelaration(_invertX(_args[0]*_speedRatio), fbr.object._getAy(),            0); return true; }
        private function _ay(fbr:CMLFiber) : Boolean { fbr.object.setAccelaration(fbr.object._getAx(),            _invertY(_args[0]*_speedRatio), 0); return true; }
        ...
        private function _gp(fbr:CMLFiber) : Boolean {
            fbr.chgt = 0;
            fbr.object.setGravity(_args[0] * _speedRatio, _args[1] * _speedRatio, _args[2]);
            return true;
        }
        
        // It's very tough to implement bulletML...('A`)
        private function _csa(fbr:CMLFiber) : Boolean { fbr.object.setChangeSpeed(_args[0]*_speedRatio,                     fbr.chgt); return true; }
        private function _csr(fbr:CMLFiber) : Boolean { fbr.object.setChangeSpeed(_args[0]*_speedRatio+fbr.object.velocity, fbr.chgt); return true; }
        private function _css(fbr:CMLFiber) : Boolean { 
            if (fbr.chgt == 0) fbr.object.setChangeSpeed(_args[0]*_speedRatio+fbr.object.velocity,          0);
            else               fbr.object.setChangeSpeed(_args[0]*_speedRatio*fbr.chgt+fbr.object.velocity, fbr.chgt);
            return true; 
        }
        private function _cd(fbr:CMLFiber) : Boolean { 
            fbr.object.setChangeDirection(fbr._getAngleForRotationCommand(), fbr.chgt, _args[0]*_speedRatio, fbr._isShortestRotation());
            return true;
        }
        // rotation
        ...
        private function _rc(fbr:CMLFiber) : Boolean {
            fbr.object.setConstantRotation(fbr._getAngleForRotationCommand(), fbr.chgt, _args[0]*_speedRatio, fbr._isShortestRotation());
            return true;
        }
        ...
        private function _bv(fbr:CMLFiber) : Boolean { fbr.bul.setSpeedStep(_args[0]*_speedRatio); return true; }
        ...
        private function _f1(fbr:CMLFiber)  : Boolean { fbr.bul.speed = _args[0]*_speedRatio; _fire(fbr, int(_args[1]), false); fbr.bul.update(); return true; }
        private function _fc0(fbr:CMLFiber) : Boolean {                                       _fire(fbr, int(_args[1]), true);  fbr.bul.update(); return true; }
        private function _fc1(fbr:CMLFiber) : Boolean { fbr.bul.speed = _args[0]*_speedRatio; _fire(fbr, int(_args[1]), true);  fbr.bul.update(); return true; }
        ...
        private function _ff1(fbr:CMLFiber) : Boolean {
            fbr.bul.speed = _args[0]*_speedRatio;
            return _ff0(fbr);
        }
    ...
    //--------------------------------------------------
        ...
        private function _new(fbr:CMLFiber, access_id:int, isParts:Boolean) : void
        {
            ...
            if (!isParts) {
                var sang:int = sin.index(fbr.object.angleOnScreen),

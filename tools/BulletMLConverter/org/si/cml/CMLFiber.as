...
1) Get the CMLFiber instance from CMLObject.execute().
  ...
  var fbr:CMLFiber    = obj.execute(seq, [1,2,3]);
    - CMLFiber is the return value of CMLObject.execute(). 
...
package org.si.cml {
    ...
    //------------------------------------------------------------
        /** @private */
        static internal var _defaultTarget:CMLObject = null;        // default target instance
    ...
    //------------------------------------------------------------
        ...
        /** @private */ internal var _pointer  :CMLState  = null;  // executing pointer
        /** @private */ internal var _access_id:int       = 0;     // access id
        ...
        // setting parameters
        /** @private */ internal var fx  :Number = 0;         // fiber position
        /** @private */ internal var fy  :Number = 0;
        /** @private */ internal var chgt:int    = 0;         // pos/vel/rot changing time
        /** @private */ internal var hopt:uint   = HO_AIM;    // head option
        /** @private */ internal var hang:Number = 0;         // head angle [degree]
        /** @private */ internal var fang:Number = 0;         // previous fired angle (due to the compatiblity with bulletML)
        /** @private */ internal var bul :CMLBarrageElem = new CMLBarrageElem();   // primary setting of bullet
        
        /** @private */ internal var invt:uint   = 0;         // invertion flag (0=no, 1=x_reverse, 2=y_reverse, 3=xy_reverse)
        /** @private */ internal var wtm1:int    = 1;         // waiting time for "w"
        /** @private */ internal var wtm2:int    = 1;         // waiting time for "~"
        /** @private */ internal var seqSub :CMLSequence = null; // previous calling sequence from "&"
        /** @private */ internal var seqExec:CMLSequence = null; // previous calling sequence from "@"
        /** @private */ internal var seqNew :CMLSequence = null; // previous calling sequence from "n"
        /** @private */ internal var seqFire:CMLSequence = null; // previous calling sequence from "f"

        // runtime parameters
        /** @private */ internal var wcnt:int   = 0;            // waiting counter
        /** @private */ internal var lcnt:Array = new Array();  // loop counter
        /** @private */ internal var jstc:Array = new Array();  // sub routine call stac
        /** @private */ internal var istc:Array = new Array();  // invertion flag stac
        /** @private */ internal var vars:Array = new Array();  // arguments
        /** @private */ internal var varc:Array = new Array();  // argument counts
        
        // head option
        /** @private */ static internal const HO_ABS:uint = 0;
        /** @private */ static internal const HO_PAR:uint = 1;
        /** @private */ static internal const HO_AIM:uint = 2;
        /** @private */ static internal const HO_FIX:uint = 3;
        /** @private */ static internal const HO_REL:uint = 4;
        /** @private */ static internal const HO_VEL:uint = 5;
        /** @private */ static internal const HO_SEQ:uint = 6;
        ...
        /** @private */ static internal var seqRapid  :CMLSequence = CMLSequence.newRapidSequence();     // rapid fire sequence
        ...
        /** @private */ static internal var _loopmax:int = 1024;
        
        // executable gosub max limitation
        /** @private */ static internal var _stacmax:int = 64;
        
        // id not specifyed 
        /** @private */ static internal const ID_NOT_SPECIFYED:int = 0;
...
<listing version="3.0">
    ...
    //------------------------------------------------------------
        ...
        internal function _destroyDestFiber(destructionStatus:int) : void
        {
            if (hasDestFiber) {
    ...
    //------------------------------------------------------------
        ...
        /** @private */ 
        internal function _onUpdate() : CMLListElem
        {
            // next fiber
        ...
        internal function _getAngle(base:Number) : Number
        {
            ...
            case HO_REL: base = _object.angleOnScreen;                   break; // based on the angle of this object
            case HO_PAR: base = _object.angleParentOnScreen;             break; // based on the angle of the parent object
        ...
        internal function _getAngleForRotationCommand() : Number
        {
            ...
            case HO_REL: return _object.angleOnScreen + hang;            // based on the angle of this object
            case HO_PAR: return _object.angleParentOnScreen + hang;      // based on the angle of the parent object
            case HO_VEL: return _object.angleVelocity + hang;            // based on the angle of velocity
            case HO_SEQ: return _object.angleOnScreen + hang * chgt;     // sequencial
            default:
                throw new Error("BUG!! unknown error in CMLFiber._getAngle()"); // ???
            }
            return 0;
    ...
    //------------------------------------------------------------
        ...
        /** @private */ 
        static internal function _onUpdate() : void
        {
        ...
        static internal function _newRootFiber(obj:CMLObject, seq:CMLSequence, args_:Array, invt_:uint) : CMLFiber
        {
        ...
        internal function _newChildFiber(seq:CMLSequence, id:int, invt_:uint, args_:Array, copyParam:Boolean) : CMLFiber
        {
        ...
        internal function _newDestFiber(seq:CMLSequence, id:int, invt_:uint, args_:Array) : CMLFiber
        {
        ...
        internal function _newObjectFiber(obj:CMLObject, seq:CMLSequence, invt_:uint, args_:Array) : CMLFiber
        {
            if (seq.isEmpty) return null;
            var fbr:CMLFiber = CMLFiber(_freeFibers.pop() || new CMLFiber());
            fbr.insert_before(_rootFiber._firstDest);                       // child of root

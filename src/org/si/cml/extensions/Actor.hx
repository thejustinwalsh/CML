//----------------------------------------------------------------------------------------------------
// Extention of CMLObject
//  Copyright (c) 2007 keim All rights reserved.
//  Distributed under BSD-style license (see license.txt).
//----------------------------------------------------------------------------------------------------
// Ported to haxe by Brian Gunn (gunnbr@gmail.com) in 2014


package org.si.cml.extensions;

//import org.si.cml.CMLSequence;


/** Extension of CMLObject that implements scope, life, hit test, drawing priority and management of instances. <br/>
 *  You have to call Actor.initialize() first, and you have to call CMLObject.frameUpdate() and Actor.draw() for each frame.<br/>
 *  Actor.initialize() registers some user define commands as below,
 *  <ul>
 *  <li>$life; Refers the value of Actor.life.</li>
 *  <li>&scon; Enables the available scope.</li>
 *  <li>&scoff; Disables the available scope.</li>
 *  <li>&prior; Changes the drawing priority. Specify the priority (posi/nega value) in argument.</li>
 *  </ul>
 */
class Actor extends ScopeLimitObject
{
// public variables
//----------------------------------------
    /** @private */
    static public var _evalLayers:Array<Actor> = new Array<Actor>();
    /** @private */
    static public var _drawLayers:Array<Actor> = new Array<Actor>();


    /** Life, you can use this as you like. */
    public var life:Float = 1;
    /** Size to use in hit test */
    public var size:Float = 0;

    /** @private */
    public var _factory:ActorFactory  = null;
    /** @private */
    public var _nextEval:Actor = null;
    /** @private */
    public var _prevEval:Actor = null;
    /** @private */
    var _nextDraw:Actor  = null;
    /** @private */
    var _prevDraw:Actor  = null;
    /** @private */
    var _evalIDNumber:Int = 0;
    /** @private */
    var _drawPriority:Int = 0;




// public properties
//----------------------------------------
    /** Enable/disable the hit test. @default true */
    public var evalEnabled(get,set) : Bool;
    public function get_evalEnabled() : Bool { return (_nextEval != this); }
    public function set_evalEnabled(enable:Bool) : Bool
    {
        _prevEval._nextEval = _nextEval;
        _nextEval._prevEval = _prevEval;
        if (enable) {
            var evalLayer:Actor = _evalLayers[_evalIDNumber];
            _prevEval = evalLayer._prevEval;
            _nextEval = evalLayer;
            _prevEval._nextEval = this;
            _nextEval._prevEval = this;
        } else {
            _nextEval = _prevEval = this;
        }

        return enable;
    }


    /** Set the drawing priority. -1 to reset as default priority */
    public var drawPriority(get,set) : Int;
    public function get_drawPriority() : Int { return _drawPriority; }
    public function set_drawPriority(prior:Int) : Int
    {
        _drawPriority = (prior == -1 && _factory != null) ? _factory._defaultDrawPriority : prior;
        var layer:Actor = _drawLayers[_drawPriority];
        if (layer == null) _drawLayers[_drawPriority] = layer = new Actor();
        if (isActive) {
            _nextDraw._prevDraw = _prevDraw;
            _prevDraw._nextDraw = _nextDraw;
            _prevDraw = layer._prevDraw;
            _nextDraw = layer;
            _prevDraw._nextDraw = this;
            _nextDraw._prevDraw = this;
        }
        return _drawPriority;
    }


    /** Set the devaluation ID Float. -1 to reset as default priority */
    public var evalIDNumber(get,set) : Int;
    public function get_evalIDNumber() : Int { return _evalIDNumber; }
    public function set_evalIDNumber(id:Int) : Int
    {
        _evalIDNumber = (id == -1 && _factory != null) ? _factory._defaultEvalIDNumber : id;
        var layer:Actor = _evalLayers[_evalIDNumber];
        if (layer == null) _evalLayers[_evalIDNumber] = layer = new Actor();
        if (isActive) {
            _nextDraw._prevDraw = _prevDraw;
            _prevDraw._nextDraw = _nextDraw;
            _prevDraw = layer._prevDraw;
            _nextDraw = layer;
            _prevDraw._nextDraw = this;
            _nextDraw._prevDraw = this;
        }
        return _evalIDNumber;
    }


    /** Factory class */
    public var actorFactory(get,null) : ActorFactory;
    public function get_actorFactory() : ActorFactory
    {
        return _factory;
    }




// constructor
//----------------------------------------
    /** Constructor */
    public function new()
    {
        super();
        _nextDraw = this;
        _prevDraw = this;
        _nextEval = this;
        _prevEval = this;
        _evalIDNumber = 0;
        _drawPriority = 0;
    }




// callback functions
//----------------------------------------
    /** @inheritDoc */
    override public function onCreate() : Void { }

    /** @inheritDoc */
    override public function onDestroy() : Void { }

    /** @inheritDoc */
    override public function onNewObject(args:Array<Dynamic>) : CMLObject { return null; }

    /** @inheritDoc */
    override public function onFireObject(args:Array<Dynamic>) : CMLObject { return null; }

    /** @inheritDoc */
    override public function onUpdate() : Void {
        // basic operation to check escaping
        if (isEscaped) destroy(0);
    }

    /** Callback function to draw. This function is called in the order of the property drawPriority. */
    public function onDraw() : Void { }

    /** Callback function from Actor.test() when the hit test is true. */
    public function onHit(act:Actor) : Void { }




// override
//----------------------------------------
    /** @private */
    override public function _initialize(parent_:CMLObject, isParts_:Bool, access_id_:Int, x_:Float, y_:Float, vx_:Float, vy_:Float, head_:Float) : CMLObject
    {
        _register();
        life = 1;
        return super._initialize(parent_, isParts_, access_id_, x_, y_, vx_, vy_, head_);
    }


    /** @private */
    override public function _finalize() : Void
    {
        _unregister();
        super._finalize();
    }




// operation for Actor list
//----------------------------------------
    /** Register this object on the actor list. */
    private function _register() : Void
    {
        if (_factory != null) {
            _evalIDNumber = _factory._defaultEvalIDNumber;
            _drawPriority = _factory._defaultDrawPriority;
        }
        var evalLayer:Actor = _evalLayers[_evalIDNumber],
            drawLayer:Actor = _drawLayers[_drawPriority];
        _prevEval = evalLayer._prevEval;
        _nextEval = evalLayer;
        _prevEval._nextEval = this;
        _nextEval._prevEval = this;
        _prevDraw = drawLayer._prevDraw;
        _nextDraw = drawLayer;
        _prevDraw._nextDraw = this;
        _nextDraw._prevDraw = this;
    }


    /** Unregister this object from the actor list. */
    private function _unregister() : Void
    {
        _nextDraw._prevDraw = _prevDraw;
        _prevDraw._nextDraw = _nextDraw;
        _prevDraw = null;
        _nextDraw = null;
        _prevEval._nextEval = _nextEval;
        _nextEval._prevEval = _prevEval;
        if (_factory != null) {
            _prevEval = _factory._freeList._prevEval;
            _nextEval = _factory._freeList;
            _prevEval._nextEval = this;
            _nextEval._prevEval = this;
        }
    }




    // operation for Actor list
    //----------------------------------------
    /** <b>Call this function first of all</b> instead of CMLObject.initialize().
     *  @param vertical_ Flag of scrolling direction
     *  @return The root object.
     *  @see Actor#onPreCreate()
     */
    static public function initialize(vertical_:Bool=true) : CMLObject
    {
        if (CMLObject.root == null) {
            CMLSequence.registerUserValiable("life",  function(f:CMLFiber)          : Float { var fiberActor : Actor = cast(f.object, Actor); return fiberActor.life; });
            CMLSequence.registerUserCommand ("prior", function(f:CMLFiber, a:Array<Dynamic>) : Void   { var fiberActor : Actor = cast(f.object, Actor); fiberActor.drawPriority = a[0]; }, 1);
        }
        return ScopeLimitObject.initialize(vertical_);
    }


    /** <b>Call this function for each frame</b> to call all onDraw()s. */
    static public function draw() : Void
    {
        var i:Int, imax:Int = _drawLayers.length, layer:Actor, act:Actor;
        for (i in 0...imax) {
            layer = _drawLayers[i];
            if (layer != null) {
                act=layer._nextDraw;
                while (act!=layer) {
                    act.onDraw();
                    act=act._nextDraw;
                }
            }
        }
    }


    /** Hit test of 2 instances. Each Actor.onHit() is called when hit test is success.
     *  @param evalID0 evaluation ID Float of 1st target ActorFactory. Optimum for slower actor.
     *  @param evalID1 evaluation ID Float of 2nd target ActorFactory. Optimum for faster actor.
     *  @see ActorFactory#evalIDFloat
     */
    static public function test(evalID0:Int, evalID1:Int) : Void
    {
        var dx:Float, dy:Float, dot:Float, sz:Float, dln2:Float, vln2:Float, dist:Float,
            act0:Actor, act1:Actor, act0_id:Int,
            list0:Actor = _evalLayers[evalID0],
            list1:Actor = _evalLayers[evalID1];

        act0=list0._nextEval;
        while (act0!=list0) {
            act0_id = act0.id;
            act1=list1._nextEval;
            while (act1!=list1) {
                dx = act0.x - act1.x;
                dy = act0.y - act1.y;
                dot = -dx * act1.vx - dy * act1.vy;
                sz = act0.size + act1.size;
                dln2 = dx * dx + dy * dy;
                if (dot<0) {
                    if (dln2 > sz*sz) continue;
                } else {
                    vln2 = act1.vx*act1.vx+act1.vy*act1.vy;
                    dot *= dot;
                    if (dot > vln2) continue;
                    dist = dln2 - dot/vln2;
                    if (dist>=0 && dist <= sz*sz) continue;
                }
                act0.onHit(act1);
                act1.onHit(act0);
                if (act0_id != act0.id) break;
                act1=act1._nextEval;
            }
            act0=act0._nextEval;
        }
    }


    /** Hit test of 2 instances, lazier and faster. Each Actor.onHit() is called when hit test is success.
     *  @param evalID0 evaluation ID Float of 1st target ActorFactory.
     *  @param evalID1 evaluation ID Float of 2nd target ActorFactory.
     *  @see ActorFactory#evalIDFloat
     */
    static public function testf(evalID0:Int, evalID1:Int) : Void
    {
        var dx:Float, dy:Float, dist:Float, act0:Actor, act1:Actor, act0_id:Int,
            list0:Actor = _evalLayers[evalID0],
            list1:Actor = _evalLayers[evalID1];

        //trace('List0: $evalID0 List1: $evalID1');
        act0=list0._nextEval;
        while (act0!=list0) {
            act0_id = act0.id;
            act1=list1._nextEval;
            //trace('Testing #$evalID0:$act0 against #$evalID1:$act1');
            while (act1!=list1) {
/*
                dx = (act0.x < act1.x) ? (act1.x - act0.x) : (act0.x - act1.x);
                dy = (act0.y < act1.y) ? (act1.y - act0.y) : (act0.y - act1.y);
                dist = (dx > dy) ? (dx + dy * 0.2928932188134524) : (dy + dx * 0.2928932188134524);
                if (dist <= act0.size+act1.size)
*/
                dx = act1.x - act0.x;
                dy = act1.y - act0.y;
                dist = act0.size + act1.size;
                //trace('dist is $dist');
                if (dist*dist > dx*dx+dy*dy) {
                    act0.onHit(act1);
                    act1.onHit(act0);
                    if (act0_id != act0.id) break;
                }
                act1=act1._nextEval;
            }
            act0=act0._nextEval;
        }
    }


    /** apply function to all */
    static public function eval(evalID:Int, func:Actor->Void) : Void
    {
        var act:Actor, list:Actor = _evalLayers[evalID];
        act=list._nextEval;
        while ( act!=list) {
            func(act);
            act=act._nextEval;
        }
    }
}

